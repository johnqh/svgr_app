...
const ORIGIN = (process.env.GENERATE_ORIGIN || 'http://localhost:5175').replace(/\/+$/, '');
...
const FILTER = (process.env.PRERENDER_FILTER || '')
  ...
  .map(s => s.trim())
  .filter(Boolean);
const PRERENDER_FLAG = '__SVGR_PRERENDER__';
...
function readSitemapPaths() {
  if (!fs.existsSync(SITEMAP)) {
    console.error(
      `[generate] sitemap not found: ${SITEMAP} — run \`bun run seo:fetch && node /tmp/generate-seo-assets.mjs public\` first.`
    );
    process.exit(1);
  }
