...
export async function savePreview(svg: string, width: number, height: number): Promise<string> {
...
import { buildTextElement, buildTextClipPath, findPathsToSuppress } from './text-overlay';
import { inpaintTextRegionsBuffer } from './text-inpaint';
import { ycbcrPipelineFromBuffer } from './ycbcr-pipeline';
import { detectImageClassificationFromBuffer, type ImageClassification } from './image-type-detect';
import {
  detectExtension,
 ...
 */
export async function runConversionPipeline(params: ConversionParams): Promise<ConversionResult> {
  const {
    imagePath,
    quality,
    smooth: smoothLevel,
  ...
  try {
    ...
    const effectiveConfig = resolveImageTypeConfig(effectiveType, classification.aiGenerated);
    const ocrConfig = effectiveConfig.ocr;
    const ocrPromise = ocr
      ? detectText(imagePath, effectiveConfig.ocrPreprocess, effectiveConfig.ocrTesseract)
        ...
        if (acceptedLines.length > 0) {
          const fontResults = await Promise.all(
            acceptedLines.map(line =>
              matchFontForLine(line, imgData, imgW, imgH, imgCh, effectiveConfig.fontMatch)
            )
          );
          ...
          for (let i = 0; i < acceptedLines.length; i++) {
            ...
            if (/&[a-z]+;/i.test(decoded)) {
              console.warn(`[ocr] Rejecting "${text}" — contains unknown HTML entities`);
              continue;
            }
            ...
            const letterDigitCount = (stripped.match(/\p{L}|\p{N}/gu) || []).length;
            const letterDigitRatio = letterDigitCount / Math.max(1, stripped.length);

            if (letterDigitCount === 0) {
              console.warn(`[ocr] Rejecting "${text}" — no letter or digit characters`);
              continue;
            }
            if (stripped.length >= 3 && letterDigitRatio < ocrConfig.lineLetterDigitRatioMin) {
              console.warn(
            ...
            if (ocrConfig.requireUniformBackground && !font.bgUniform) {
              console.warn(`[ocr] Skipping "${text}" — non-uniform background`);
              continue;
            }
    ...
    const traceInputPath = useYcbcrReduction
      ? await ycbcrPipelineFromBuffer(vectorizeData, imgW, imgH, quality, effectiveConfig.ycbcr)
      : refineImagePath;
    ...
    const shouldRefine = shouldRunColorRefinement(effectiveConfig, verifiedLines.length);
    ...
    if (verifiedLines.length > 0) {
      ...
      if (suppressSet.size > 0) {
        ...
        let pathIdx = 0;
        svg = svg.replace(pathRegex, match => (suppressSet.has(pathIdx++) ? '' : match));
      }
    ...
    if (shouldApplySvgBlur(smoothLevel, effectiveConfig)) {
      ...
      svg = svg.replace(/(<svg[^>]*>)/, `$1${filterDef}<g filter="url(#smooth)">`);
...
async function ensureUserExists(firebaseUid: string, email?: string | null) {
  const existing = await db.select().from(users).where(eq(users.firebase_uid, firebaseUid));
...
routes.post(
  ...
  async c => {
    const { original, quality, smooth, transparentBg, ocr, mergePaths, imageType } =
      c.req.valid('json');
...
export async function processJob(jobId: string): Promise<void> {
  try {
    ...
    const [image] = await db.select().from(images).where(eq(images.id, job.image_id));
...
import sharp from 'sharp';
import { errorResponse, successResponse, type ImageUploadResult } from '@sudobility/svgr_types';
...
imageRoutes.post('/upload', requireAuthMiddleware, async c => {
  try {
...
import { errorResponse, successResponse, type JobResult } from '@sudobility/svgr_types';
import { requireAuthMiddleware } from '../middleware/requireAuth';
import { db, images, jobs } from '../db';
...
// Get job status
jobRoutes.get('/jobs/:jobId', requireAuthMiddleware, async c => {
...
jobRoutes.get('/jobs', requireAuthMiddleware, async c => {
...
fileRoutes.get('/files/:filename', requireAuthMiddleware, async c => {
...
async function checkFileOwnership(filename: string, userId: string): Promise<boolean> {
...
import { successResponse, errorResponse, type HealthResult } from '@sudobility/svgr_types';
...
describe('SvgrClient job methods', () => {
  ...
  it('getJobsForImage sends GET with imageId query', async () => {
    ...
    expect(result.success).toBe(true);
    expect(mockNetwork.wasUrlCalled('http://localhost:3001/api/v1/jobs?imageId=i1', 'GET')).toBe(
      true
    );
  });
});
...
export function useUploadImage(client: SvgrClient) {
  return useMutation({
    mutationFn: (file: File | { buffer: ArrayBuffer; filename: string; mimeType: string }) =>
...
export const TRANSPARENT_BG_SUPPORTED_IMAGE_TYPES: ReadonlySet<ImageType> = new Set([
  'auto',
  'design',
  'line_art',
  'logo',
...
export type ScaleImageFn = (base64: string, maxPixels: number) => Promise<string>;
...
export function useImageConverter(
    ...
    createJobMutation
      ...
      .then(response => {
        if (response.success && response.data) {
          ...
          setError((response as { error?: string }).error ?? 'Failed to create job');
        }
      })
      .catch(err => {
  ...
  const isConverting =
    ...
    (currentJob !== null && currentJob.status !== 'done' && currentJob.status !== 'error');
